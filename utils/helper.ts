...
export default function paginationEllipsis(currPage: number, total : number, numDisplay : number){
  let pageNumbers = Array.from(Array(total).keys());
  ...
  // When the current page is greater than the display count, add ellipsis and numbers around current page
  if(currPage > numDisplay) return pageNumbers.slice(currPage - left, currPage + right);
  ...
  if(currPage - left <= 0) return pageNumbers.slice(0, numDisplay);
}